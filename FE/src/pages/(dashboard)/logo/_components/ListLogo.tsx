...
const ListLogoPage = () => {
  ...
  const handleSetCurrent = async (id: string) => {
    try {
      await axios.put(`http://localhost:8080/api/logo/current/${id}`);
      const response = await axios.get("http://localhost:8080/api/logo");
      setLogos(response.data);
      toast.success("Đặt làm logo chính thành công!");
    } catch (error) {
      console.error("Lỗi khi đặt làm logo chính.", error);
      toast.error("Đặt làm logo chính thất bại!");
    }
  };
  ...
  return (
    <div className="mx-auto p-4 md:p-8">
      <div className="flex flex-col md:flex-row items-center justify-between mb-8">
        ...
        <Link
          to="/dashboard/logos/add"
          className="mt-4 md:mt-0 flex items-center gap-2  md:w-44 bg-blue-500 text-white px-4 py-2 rounded-2xl hover:bg-blue-600 shadow-lg transition duration-200"
        >
          <svg
            ...
            >
              ...
              <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                <img
                  src={logo.image}
                  alt={logo.title}
                  className="w-32 h-20 md:w-40 md:h-24 rounded-lg object-cover shadow-sm"
                />
              </td>
              <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                <button
              ...
              <td className="px-4 md:px-6 py-4 whitespace-nowrap">
                <div className="flex space-x-2">
                  <Link
                    to={`/dashboard/logos/edit/${logo._id}`}
